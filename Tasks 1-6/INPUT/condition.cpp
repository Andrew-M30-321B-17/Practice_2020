...
string Condition::api(string comand){

    URI uri("http://127.0.0.1:5000/api/"+comand);
    std::string path(uri.getPathAndQuery());
    if (path.empty()) path = "/";
    HTTPClientSession session(uri.getHost(), uri.getPort());
    HTTPRequest request(HTTPRequest::HTTP_GET, path, HTTPMessage::HTTP_1_1);
    HTTPResponse response;
    //string path = "http://127.0.0.1:5000/api/v1/resources/books/all";
    session.sendRequest(request);
        istream& rs = session.receiveResponse(response);
        char ans[255]; string  answer = "";
        rs.getline(ans,255);
        cout << response.getStatus() << "  " << response.getReason() << " "<<  ans << endl;
        answer += ans;
...
void Condition::insertAPI(int id){

    string url = "add/?id="+to_string(id)+"&timest="+to_string(time(NULL))+"&tempIn="+to_string(tempIn)+"&tempOut="+to_string(tempOut)+"&xPos="+to_string(xPos)+"&yPos="+to_string(yPos)+"&speed"
            "="+to_string(speed)+"&acceleration="+to_string(acceleration)+"&fuel="+to_string(fuel)+"&conditioner="+to_string(conditioner)+"&refueling="+to_string(refueling)+"&length"
                      "Way="+to_string(lengthWay)+"&timeOfWay="+to_string(timeOfWay);
    this->api(url);
}

void Condition::newsel()
{
    cout << "work" << endl;
    clientDB->Select("SELECT id, speed FROM Practice.condition", [] (const Block& block)
        {
            for (size_t i = 0; i < block.GetRowCount(); ++i) {
