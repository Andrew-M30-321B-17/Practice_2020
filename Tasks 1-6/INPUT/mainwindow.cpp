...
void MainWindow::truncate(){
    string api;
    if(ui->checkBox_2->isChecked())api = "api"; else api = "noapi";
    thcl *thread= new thcl(ui,0,"trunc",&vecVals,&vecNames,0,api);
...
void MainWindow::drop(){
    string api;
    if(ui->checkBox_2->isChecked())api = "api"; else api = "noapi";
    thcl *thread= new thcl(ui,0,"drop",&vecVals,&vecNames,0,api);
        connect(thread, SIGNAL(send(int)), this, SLOT(update(int)));
    thread->start();
...
void MainWindow::addSimple(){
    ...
    thcl *thread= new thcl(ui,0,"addSimple",&vecVals,&vecNames,1,"noApi");
        connect(thread, SIGNAL(send(int)), this, SLOT(update(int)));
    thread->start();
}
...
void MainWindow::hashSlot(){
    ...
    if(ui->checkBox->isChecked()){
        ...
        if(ui->lineEdit_12->text() == "") ui->label_22->setText(ui->label_22->text()+QString::fromStdString("\r\nВведите имя топика"));
                ...
                ui->label_22->clear();
                thcl *thread= new thcl(ui,1,"mqtt",&vecVals,&vecNames,1,"noApi");
                    connect(thread, SIGNAL(send(int)), this, SLOT(update(int)));
                thread->start();
        ...
        if(ui->pushButton->isEnabled()){
            ...
            thcl *thread= new thcl(ui,0,"run",&vecVals,&vecNames,1,api);
                connect(thread, SIGNAL(send(int)), this, SLOT(update(int)));
            thread->start();
            ...
            thcl *thread= new thcl(ui,1,"stop",&vecVals,&vecNames,0,api);
             connect(thread, SIGNAL(send(int)), this, SLOT(update(int)));
            thread->start();
